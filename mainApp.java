			...
			if (answer.equals ("1")){
				...
				if (Exists(thing, shopStock)){  
					...
					if ( decision.equals("Y")){
						...
						if(str.equals("Y")){
							if (str1.equals("1")){
								fp = thing.getPrice() - thing.getPrice()* HWSale/100;                  
								System.out.println("The final price is : "+ fp +" Euros.");
							}else if(str1.equals("2")){
								fp = thing.getPrice() - thing.getPrice() * peripheralSale/100;                  
								System.out.println("The final price is : "+ fp +" Euros.");
							}
						}else{
							fp = thing.getPrice();
		...
		if (ordersList.size() != 0){
			...
			while(true){
				if ( reply==0){
					...
					if(response.equals("Y")){//  E   D  W  ALLAKSA    TO str me to response kati dn mou paei kala
						status = true;
